...
- **Leaderboard notification toast**: always shown on login (not gated on `scoresHaveChanged()`). `getRankMessage(pid)` returns `{ msg, toastCls }`. `toastCls` is `'toast-positive'` (green) for moved-up or top-3, `''` (orange accent) for no change ("You're holding Xth place 🔒"), `'toast-negative'` (red) for moved down. Both `showIncompleteToast` (My Picks) and `leaderboardMessage` / `leaderboardToastCls` (Leaderboard) are set at login independently — each notification persists until the user navigates away from its tab (`switchTab()` clears them), not on re-render. Leaderboard is initially sorted alphabetically when all scores are 0.
...
- **R1 recap page**: `r1-recap.html` — standalone static page with leaderboard, series results, key insights, and full pick grid for Round 1. Accessible at `galfogel.github.io/NBA-Bracket/r1-recap.html`. No app.js dependency; all data is hardcoded HTML.
