...
- **Pick Details — Finals extra tables**: When `bdRoundFilter === 4`, two additional tables appear below the Finals series card in a side-by-side `.breakdown-finals-row` flex container: (1) **Correct Picks** — always visible; sorted by leaderboard order (not by correct count); shows `X/total` format. (2) **Game 1 Finals Gap** — gaps hidden with `🔒 Hidden until series starts` until `isSeriesLocked('FINALS') || adminView`; once revealed shows each user's gap in pts or `?` if not entered; diff shown after actual gap is known (always green, `.bd-diff-close/mid/far` all use `var(--green)`). Both tables use `.breakdown-finals-gap` (full-width single-column grid). Side-by-side on all screen sizes; `flex: 1 1 0` ensures equal widths.
