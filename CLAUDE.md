...
- **Pick Details conference layout**: R1 — East then West stacked. R2 (CSF) — side by side desktop only. R3 (CF) — side by side on both desktop and mobile (`.breakdown-confs-row--r3` overrides the mobile stack). R4 (Finals) — single card centered (`.breakdown-finals-centered`, `max-width: 320px`). Wrapper gets `breakdown-confs-row--r${bdRoundFilter}` class for per-round CSS targeting.
