...
- **Best possible placement** (`renderMaxPlacements(rows)`): shown below the leaderboard table as a single text line (`.max-place-section`, `margin: 6px 0`, `font-size: 13px`, dim text). Only visible while at least one series is still open. Anchors the scenario to the **current user's picks** — everyone's score is computed assuming MY picks are the actual outcomes. This is intentional: computing each player's independent best case produces an impossible scenario (e.g. SAS winning in both 6 and 7 simultaneously). Games bonus is awarded only when another player's games pick matches mine AND the count is still achievable given the current live series record (`getRecord` → `oppWins + 4` check). Includes FINALS once the user has made a FINALS pick; excluded until then. Updates automatically every 5 min as `scoresData` refreshes.
...
- **Pick Details — Finals extra tables**: When `bdRoundFilter === 4`, two additional tables appear below the Finals series card, both using `.breakdown-finals-gap` (full-width single-column grid): (1) **Correct Picks Leaderboard** — always visible; sorted by `correct` descending then name ascending; dense ranking (1,1,2,2 — rank increments only when score changes, never skips); medals 🥇🥈🥉 for top 3; shows `X/total` format. (2) **Game 1 Finals Gap (Tiebreaker)** — gaps hidden with `🔒 Hidden until series starts` until `isSeriesLocked('FINALS') || adminView`; once revealed shows each user's gap in pts or `?` if not entered; diff shown after actual gap is known.
- **My Picks pending toast**: Shows "You have pending picks — lock them in before tip-off! ⏰" when any available + unlocked series has no winner or games pick. No hard save requirements — all picks are optional; gap is never required for saving.
